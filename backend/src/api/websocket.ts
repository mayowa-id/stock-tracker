...
export function createWebSocketRouter(upgradeWebSocket: (handler: (c: Context) => any) => (c: Context, next: Next) => Promise<Response | void>) {
  const app = new Hono();
  app.get('/:symbol', upgradeWebSocket((c: Context) => {
    ...
    return {
      ...
      onError: (err: Error, ws: WebSocket) => {
